...
<?foreach($arResult["ITEMS"] as $key=>$arItem):?>
		<div class="bx_slider_block bx_index_number_<?=$key+1?> <?if ($key==0):?>active<?endif?>" style="width:33.333%;">
			<div class="bx_slider_photo_container">
				...
				<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="bx_slider_photo_element" style="background: #ffd url=('<?=($arItem["DETAIL_PICTURE"]["SRC"])?>') no-repeat center;">

<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="bx_slider_photo_element" style="background: #fff">
...
<div>
   <h1 class="bx_slider_title"><a href="<?=$arItem["DETAIL_PAGE_URL"]?>"><?=$arItem["NAME"]?></a>
</h1>
</div>
				<div class="bx_slider_content_description" margin-top: 25px; padding-top: 25px;">
                                  <?=$arItem["PREVIEW_TEXT"] ? $arItem["PREVIEW_TEXT"] : $arItem["DETAILTEXT"]?>
                                </div>

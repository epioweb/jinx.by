...
{
//console.log(arParams.TREE_PROPS.length);
	this.productType = 0;
	...
	if (0 == this.errorCode)
	{
		 BX.ready(BX.delegate(this.Init('li')));
                 BX.ready(BX.delegate(this.Init('option')));
	}

};

JCCatalogElement.prototype.Init = function(skuTag)
{
	var i = 0;
	var j = 0;
...
/*	if (!!this.visual.ZOOM_DIV && !!this.visual.ZOOM_PICT)
	...
	if (0 == this.errorCode)
 	{
           if('option' != skuTag) //чтобы не считался товар дважды при выставлении количества
           {
		if (this.showQuantity)
		{
...
//Обработчик cмены класса при выборе(выбор размера, цвета)
JCCatalogElement.prototype.SelectOfferProp = function(e,skuTag)
{
   if (!e) e = window.event;
      var target = BX.proxy_context;
...
//console.log(this.treeProps);
	...
	if (-1 < index)
	{
		...
		for (i = index+1; i < this.treeProps.length; i++)
		{
			...
			this.UpdateRow(i, arFilter[strName], arShowValues, arCanBuyValues, 'li');
                        this.UpdateRow(i, arFilter[strName], arShowValues, arCanBuyValues, 'option');
		}
...
//Обработка вывода свойств(внешний вид)
JCCatalogElement.prototype.UpdateRow = function(intNumber, activeID, showID, canBuyID, propTag)
{
	var i = 0;
	var value;
	var countShow = 0;
	...
	if (-1 < intNumber && intNumber < this.obTreeRows.length)
	{
		...
		if (!!RowItems && 0 < RowItems.length)
		{
			...
			strNewLen = (5 < countShow ? (100/countShow)+'%' : '100%');
			obData = {
				props: { className: '' },
				style: {
					width: '100%' /*strNewLen*/
...
JCCatalogElement.prototype.ChangeInfo = function()
{
	...
	if (-1 < index)
	{
		...
		//BX.onCustomEvent('onCatalogStoreProductChange', [1274]);
	}
};

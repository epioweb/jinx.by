...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	foreach ($arResult['OFFERS'] as &$arOffer)
	{
		...
		unset($strOneCode);
                ...
                // show if != 'no_color'
                if($arOffer["PROPERTIES"]["COLOR_REF"]["VALUE"] != 'no_color'):
                   $arResult["SHOW_COLOR"] = 'Y';
                endif;
	}
	unset($arOffer);
...
<?
//Catalog.element - result_modifier
$res=CIBlockElement::GetByID($arResult["PROPERTIES"]["BLINDING_TO_EL"]["VALUE"]);

while($ar_res = $res->GetNextElement()):
   $fields = $ar_res->GetFields();
   $arResult["PROPERTIES"]["BLINDING_TO_EL"]["DETAIL_PAGE_URL"] = $fields["DETAIL_PAGE_URL"];
   
   $props = $ar_res->GetProperties();
   $arResult["PROPERTIES"]["BLINDING_TO_EL"]["CATEGORY"] = $props["CATEGORY"]["VALUE"];   
endwhile;
?><?
//Catalog.element - result_modifier
$res=CIBlockElement::GetByID($arResult["PROPERTIES"]["BLINDING_TO_EL"]["VALUE"]);
...
/*
//Resizing of detail images
foreach($arResult[OFFERS] as $key1 => $arOffer):
   foreach($arOffer["MORE_PHOTO"] as $key2 => $arPhoto):
      ...
      $arResult[OFFERS][$key1]["MORE_PHOTO"][$key2]["WIDTH"] = $image["width"];
      $arResult[OFFERS][$key1]["MORE_PHOTO"][$key2]["HEIGHT"] = $image["height"];

*/

//Отображение статуса ПОД ЗАКАЗ в размерной сетке
foreach($arResult[OFFERS] as $key => $arOffer):
   $size = $arOffer["PROPERTIES"]["SIZES_CLOTHES"]["VALUE"];
   if($arOffer["PROPERTIES"]["ORDER"]["VALUE"] == 'Y'):
      foreach($arResult[SKU_PROPS] as $key1 => $arProps):
         if(substr($arProps["CODE"],0,5) == "SIZES"):
            if($arOffer["PROPERTIES"]["ORDER"]["VALUE"] == "Y"):
                 foreach($arProps["VALUES"] as $key2 => $arValue):
                    if($arValue["NAME"] == $size):
                       $arResult[SKU_PROPS][$key1]["VALUES"][$key2]["SHOW_ORDER"] = "под заказ 2-3 недели";
                    endif;
               endforeach;
            endif;
         endif;
...
//Удаляется цвет "Не установлен", чтобы не отображалась пустая картинка
unset($arResult["SKU_PROPS"][0]["VALUES"][15]);
?>

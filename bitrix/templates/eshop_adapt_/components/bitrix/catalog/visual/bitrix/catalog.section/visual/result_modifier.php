...
foreach($arResult["ITEMS"] as $itemKey => $arItem):
   foreach($arItem["OFFERS"] as $offerKey => $arrOffer):
      if($arrOffer["PROPERTIES"]["ORDER"]["VALUE"] == "Y" && $arrOffer["PROPERTIES"]["SIZES_CLOTHES"]["VALUE"] == $arValues[$arEnum['ID']]["NAME"]):
         $arValues[$arEnum['ID']]["POD_ZAKAZ"][]=$arrOffer["ID"];
      endif;
   endforeach;
endforeach;
